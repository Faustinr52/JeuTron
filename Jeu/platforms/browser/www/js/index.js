...
var joueur2;
 ...
 var sendPseudo = function(){
  ...
  var gagnePartie = function() {
      socket.emit("fin partie", true);
  }

  var perdPartie = function() {
      socket.emit("fin partie", false);
  }
 ...
 var receiveStats = function(msg){
  var infos = JSON.parse(msg);
    joueur = infos["Pseudo"] ;
    document.getElementById('infos').innerHTML = 'Pseudo : '+ infos["Pseudo"] +' Victoire : '+infos["nbVictoire"]+' Parties jouées : '+infos["nbPartie"] +"  ";
    console.log(msg);
 }
...
var receiveDp = function(msg){
  ...
  document.getElementById("j1").style.color = "#8B0000";
  document.getElementById("j2").innerHTML = joueur2;
  document.getElementById("j2").style.color = "#FF1493";
}
  ...
  update() {
    for (let i = 0; i < this.cycles.length; i++) {
      ...
      if (!this.playerShouldDie(cycle) && !this.playerShouldDie(cycle2)) {
        ...
        cycle.position = previousPosition;
        cycle.active = false;
        this.updateCell(cycle.position, cycle.color);
        
        cycle2.position = previousPosition2
        cycle2.active = false;
        this.updateCell(cycle2.position, cycle2.color);
        ...
        if(this.playerShouldDie(cycle)){
          ...
          var gagne;
          if(Numjoueur == 1){
            gagne = players[0].gagne == true;
          } else {
            gagne = players[1].gagne == true;
          }
